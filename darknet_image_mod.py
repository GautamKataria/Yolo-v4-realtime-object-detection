...
def YOLO(in123, in1234):
    ...
    while not keyboard.is_pressed('q'):
        ...
        #in123 = input("Enter name of jpg image saved in data folder without its extension: ")
        ...
        #cap = cv2.VideoCapture("office.mp4")                           #Local Stored video detection - Set input video
        #image_path = r'C:\Users\Gautam\Desktop\darknet\darknet\build\darknet\x64\data\img123'
        cap = cv2.imread(F'./data/{in123}.jpg',3)
        ...
        #while True:                                                      # Load the input frame and write output frame.
         #   prev_time = time.time()
        #frame_read = cap                                 # Capture frame and return true if frame present
            # For Assertion Failed Error in OpenCV
           # if not ret:                                                  # Check if frame present otherwise he break the while loop
            #    break

        frame_rgb = cv2.cvtColor(cap, cv2.COLOR_BGR2RGB)      # Convert frame into RGB from BGR and resize accordingly
        ...
        cv2.imshow('Detections', image)                                    # Display Image window
        cv2.waitKey(0)
        cv2.destroyAllWindows()
        #cv2.waitKey(3)
        cv2.imwrite(F'./Demo/{in1234}.jpg', image)
